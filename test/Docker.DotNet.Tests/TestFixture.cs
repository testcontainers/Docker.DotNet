...
public sealed class TestFixture : Progress<JSONMessage>, IAsyncLifetime, IDisposable, ILogger
{
    ...
    public TestFixture(IMessageSink messageSink)
    {
        ...
        {
            { TestClientsEnum.ManagedPipe, new DockerClientConfiguration().CreateClient(null, OperatingSystem.IsWindows() ? new NpipeHandlerFactory() : new UnixHandlerFactory(), logger: this) },
            { TestClientsEnum.ManagedHttp, new DockerClientConfiguration(endpoint: new Uri("http://localhost:2375")).CreateClient(null, new LegacyHttpHandlerFactory(), this) },
            { TestClientsEnum.NativeHttp, new DockerClientConfiguration(endpoint: new Uri("http://localhost:2375")).CreateClient(null, new NativeHttpHandlerFactory(), logger: this) },
        };
...
#endif
            DockerClients.Add(TestClientsEnum.ManagedHttps, new DockerClientConfiguration(endpoint: new Uri("http://localhost:2376"), credentials).CreateClient(null, new LegacyHttpHandlerFactory(), this));
            DockerClients.Add(TestClientsEnum.NativeHttps, new DockerClientConfiguration(endpoint: new Uri("http://localhost:2376"), credentials).CreateClient(null, new NativeHttpHandlerFactory(), logger: this));
        }
        catch (Exception ex)
        {
